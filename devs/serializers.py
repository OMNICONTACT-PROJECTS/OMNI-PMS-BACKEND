...
class DevSerializer(ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        user.is_staff = True
        user.is_active = True
        # total_devs = Dev.objects.filter(
        #     user__organisation_id=user_data["organisation"]
        # ).count()
        new_username = f"{user.username}DEV"
        user.username = new_username
        user.set_password("omni-developers-123")

        try:
            dev = Dev.objects.create(user=user, **validated_data)
            user.save()

            groups = Group.objects.filter(
                id__in=[group_data["id"] for group_data in groups_data]
            )
            dev.user.groups.set(groups)

            permissions = Permission.objects.filter(
                id__in=[perm_data["id"] for perm_data in permissions_data]
            )
            dev.user.user_permissions.set(permissions)

            dev.save()
            return dev

        except Exception as e:
            user.delete()  # Delete the user if dev creation fails
...
class DevRetrieveSerializer(ModelSerializer):
